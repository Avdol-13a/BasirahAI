...
└── app/                              ← Flutter project — BUILT, `flutter analyze` clean, APK builds successfully
    ├── lib/
    │   ├── main.dart                 ← Supabase.initialize, MaterialApp, AuthGate routing
    │   ├── config/
    ...
    │   ├── services/
    │   │   └── inference_service.dart← Dio client for POST /screen, typed error messages
    │   └── screens/
    │       ├── auth/
    ...
    │       │   └── patient_detail_screen.dart← shows patient info + screening history + "New Screening" button
    │       └── screening/
    ...
    └── pubspec.yaml                  ← dio, supabase_flutter, image_picker, image
```
...
2. **Run the independent sanity-check evaluation:**
   - Run an independent sanity-check evaluation: pull ~150–300 real APTOS sample images (via Kaggle, same dataset documented in `docs/DATASET.md`) through the live `/screen` endpoint, compute real sensitivity/specificity/confusion-matrix numbers, and fill them into `docs/EVALUATION_RESULTS.md` — **clearly labeled as this team's own measurement, separate from the model author's self-reported accuracy** (do not blend the two, do not fabricate numbers).
   - Tune the low-confidence cutoff (currently 0.6 placeholder) based on that real data; update it consistently in `app/lib/models/result_category.dart`, `docs/ML_PLAN.md`, and `docs/MEDICAL_SAFETY.md`.
3. **Urdu/RTL localization — not started for the current architecture.** Apply it across login, signup, patient, screening, result, and history screens.
