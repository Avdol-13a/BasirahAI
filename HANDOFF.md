...
5. **Release APK build + real-device testing over real mobile data — NEXT UP, not started.** Explicitly chosen to do this even though the plan is to demo live (not distribute an installable file): a **debug** build shows a red "DEBUG" ribbon across every screen and runs the JIT-interpreted Dart VM (visibly slower/janker) versus release's clean, AOT-compiled build — given the effort that went into the visual redesign, demoing in debug mode would undercut it. This is a low-cost step: same build command with `--release` instead of `--debug`; no real signing setup needed since the default debug keystore is fine for local install (not distributing via Play Store). Exact command (adapt from the debug one in §10, just swap `--debug` → `--release`):
   ```bash
   cd D:/Basirah/app
   flutter build apk --release \
     --dart-define=SUPABASE_URL=https://cqpoochxfbjflaxzsfce.supabase.co \
     --dart-define=SUPABASE_ANON_KEY=<see §10 for the full key>
   ```
   Then install on a **real Android phone** (not the emulator) connected via USB with `adb install -r build/app/outputs/flutter-apk/app-release.apk`, and run through the full flow (login → patient → screening → result, in both English and Urdu) over **real mobile data**, not Wi-Fi — this is the one thing that genuinely hasn't been tested yet (everything so far has been the Android emulator, which is Wi-Fi-only). Do a quick smoke test specifically because release-mode code shrinking (R8/ProGuard) can occasionally behave differently from debug even though Flutter apps are usually unaffected.
6. **Demo rehearsal**, including the documented limitations (not clinically validated, the evaluation-numbers training-overlap caveat in `docs/EVALUATION_RESULTS.md`, MVP-level PII handling) and a backend + Supabase warm-up shortly before judging.
